...
bool FoldLine::addCtrlPt(glm::f64vec3& p, int dim, OUTLINE *outline, std::vector<Face*>& Faces, std::vector<HalfEdge*>&Edges, std::vector<Vertex*>& Vertices){
    if((int)CtrlPts.size() <= dim)CtrlPts.push_back(p);
    else{
        if(he != nullptr){
            vx2 = new Vertex(p); he2 = new HalfEdge(vx2);
            he2->prev = he; he->next = he2;
        }
        if(he == nullptr){
            vx = new Vertex(p); he = new HalfEdge(vx);
        }
    }
    bool hasCrv = setCurve(dim);
    /*
...
bool FoldLine::applyCurvedFolding(std::vector<Face*>& Faces, std::vector<HalfEdge*>& Edges, std::vector<Vertex*>& Vertices, int dim){
    ...
    for(int i = 1; i < Csize; i++){
        devide(_CrvPts[i - 1], _CrvPts[i], Faces, Edges);
    }
    return true;
}
...
bool FoldLine::modify2DRulings(std::vector<Face*>& Faces, std::vector<HalfEdge*>& Edges, std::vector<Vertex*>& Vertices,int dim){
    glm::f64vec3 d, dr, dr2, dr3, T, T2d, N, B;
    glm::f64vec3 dh_p,dh_2p, dh_m, dh_2m, dh_3p, dh_3m, crossPts;
    double k2d, k3d, tau, k3d_bef, k2d_bef;
    auto rad_2d = [](double k, double tau, double a, double da) { return std::isfinite(k*sin(a)/(da + tau)) ? atan(tan(k*sin(a)/(da + tau))) : M_PI/2; };
    ...
    if(type == 0){
        ...
        for(int i = 0; i < EdgeSize; i++){
            ...
            for(auto&t: arcT){
                ...
                NewInsertedEdges.push_back(he_new[0]); NewInsertedEdges.push_back(he_new[1]);

                dr = -3. * (1 - t) * (1 - t) * CtrlPts[0] + (9 * t * t - 12 * t + 3) * CtrlPts[1] + (-9 * t * t + 6 * t) * CtrlPts[2] + 3 * t * t * CtrlPts[3];
                dr2 = 6. * (1 - t) * CtrlPts[0] + (18 * t - 12) * CtrlPts[1] + (-18 * t + 6) * CtrlPts[2] + 6 * t * CtrlPts[3];
                k2d = glm::length(glm::cross(dr, dr2))/std::pow(glm::length(dr), 3);//ここ違う気がする
                k2d = glm::length(dr2);
                ...
                N = glm::normalize(glm::cross(dr, glm::cross(dr2, dr)));
                B = glm::normalize(glm::cross(dr, dr2));
                //std::cout<< "current " <<  i << "  :  T = " << glm::to_string(T) << " , N = " << glm::to_string(N) << " , B = " << glm::to_string(B) << " , T2d " << glm::to_string(T2d) <<  std::endl;
                if(glm::dot(glm::f64vec3{0,0,-1}, B) < 0){B *= -1; N *= -1;}
                tau = glm::dot(dr, glm::cross(dr2, dr3))/std::pow(glm::length(glm::cross(dr, dr2)), 2);
                k3d = glm::length(glm::cross(dr, dr2))/std::pow(glm::length(dr), 3);
                k3d = glm::length(dr2);
                k2d = (k2d <= k3d) ? k2d : k3d;
                a = (k3d != 0) ? acos(k2d/k3d): acos(0);
                {//bef_aの計算
                    double t2 = t - eps;
                    ...
                    k3d_bef = glm::length(dr2);
                    k2d_bef = (k2d_bef <= k3d_bef) ? k2d_bef: k3d_bef;
                    a_bef = (k3d_bef != 0) ? acos((k2d_bef/k3d_bef)) : acos(0);
                }
                //std::cout<< "bef " <<  i << "  :  T = " << glm::to_string(T) << " , N = " << glm::to_string(N) << " , B = " << glm::to_string(B) << " , T2d " << glm::to_string(T2d) <<  std::endl;
                ...
                glm::f64vec3 rl2d = glm::normalize(glm::rotate(phi_bl, glm::f64vec3{0,0,1}) * glm::f64vec4{T2d,1});
                glm::f64vec3 rr2d = glm::normalize(glm::rotate(-phi_br, glm::f64vec3{0,0,1}) * glm::f64vec4{T2d,1});
                bool res = setPoint(Faces, rl2d, pt_new,crossPts);
                if(res){
                    double l = glm::distance(crossPts, pt_new);
                    //std::cout<< glm::to_string(rl3d) << " , " << glm::to_string(rl2d) << " , " << glm::to_string(crossPts) << " ,  " << l <<  std::endl;
                    if(glm::dot(rl2d, glm::normalize(he_new[0]->next->vertex->p - pt_new)) > 0){
                        he_new[0]->next->vertex->p3 = rl3d * l + pt_new;
                        he_new[0]->next->vertex->p = crossPts;
                    }else{
                        he_new[1]->next->vertex->p3 = rl3d * l + pt_new;
                        he_new[1]->next->vertex->p = crossPts;
                    }
                }
                bool res2 = setPoint(Faces, rr2d, pt_new,crossPts);
                if(res2){
                    double l = glm::distance(crossPts, pt_new);
                    if(glm::dot(rr2d, glm::normalize(he_new[1]->next->vertex->p - pt_new)) > 0){
                        he_new[1]->next->vertex->p3 = rr3d * l + pt_new;
                        he_new[1]->next->vertex->p = crossPts;
                    }else{
                        he_new[0]->next->vertex->p3 = rr3d * l + pt_new;
                        he_new[0]->next->vertex->p = crossPts;
                    }
                }
...
std::array<HalfEdge*, 2> FoldLine::splitOnPoint(Vertex *v, std::vector<HalfEdge*>& Edges){
    ...
    for(auto&h: Edges){
        if(is_point_on_line(v->p, h->vertex->p, h->next->vertex->p)){
            HalfEdge *h_new = new HalfEdge(v);
            if(h->pair != nullptr){
                HalfEdge *_pair = h->pair;
                double t = glm::length(v->p - h->vertex->p)/glm::length(_pair->vertex->p - h->vertex->p);
                v->p3 = h->vertex->p3 + t * (_pair->vertex->p3 - h->vertex->p3);
                //std::cout<<"split " << t << "   ,  " << glm::to_string(h->vertex->p3) << glm::to_string(_pair->vertex->p3) <<  glm::to_string(v->p3) <<std::endl;
                HalfEdge *h2_new = new HalfEdge(v);
...
void FoldLine::devide2Faces(std::vector<HalfEdge*>& Inserted, std::vector<HalfEdge*>& Edges, std::vector<Face*>& Faces){
    ...
    for(int i = 0; i < (int)Inserted.size(); i++){
        ...
        HalfEdge *h_new = new HalfEdge(Inserted[i]->vertex);
        HalfEdge *h2_new = new HalfEdge(he_pair->vertex);
...
void FoldLine::devide(Vertex *v1, Vertex *v2, std::vector<Face*>& Faces, std::vector<HalfEdge*>& Edges){
    for(auto&f: Faces){
        ...
        if(h1 != nullptr && h2 != nullptr){
            HalfEdge *h1_new = new HalfEdge(h1->vertex), *h2_new = new HalfEdge(h2->vertex);
